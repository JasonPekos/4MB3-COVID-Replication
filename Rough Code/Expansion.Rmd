---
title: "Expansion"
author: "Zachary Levine, Jason Pekos,  Ariel Pillitteri"
date: "08/04/2021"
output: pdf_document
---
...
It turns out our fitted infection curves totally failed to capture the underlying dynamics in the simpler model. This is largely because --- following the initial spike in deaths during the spring --- transmisibility stayed low due to public panic, only for it to creep up again in the fall. This is not captured well in a standard SIR model, and so this extension is about quantifying this problem and dealing with it. \
...
The next few code chunks provide a nice framework for plotting the CN covid data by town, with plenty of options. This is meant for checking spacial heterogeniety --- as mentioned in meetings, we looked a lot at how we could get better and more accurate model fit, using results from the King et al paper. This paper notes that in pandemics with high spacial heterogeniety, models fail to predict pandemic outcomes due to many small local flairups vitiating the usual logic around herd immunity in an SIR model, which requires a high level of mixing to be accurate. (Note, some of these code chunks moved to the start to fascilitate the flow of this overview, specifically the python plotting).\
...
\
Next, we produce plots to check spacial heterogeniety for some of the larger towns:\
```{python}
#Plots
...
\
Our fitted parameters are below. Note that they are just the initial parameters, as our method did not converge:
\
```{r}
...
Now checking susceptables to see if persistant deaths are due to vital dynamics or disease dynamics
\
```{python}
plt.title("Remaining Susceptables in 70-89: Model Output")
plt.plot((res["S5"]))
```
\
This suggests deaths are due to disease dynamics, not natural death rate --- should look at changing $\delta$, but no bugs here. 
\
...
\
\
Our next steps include fixing these numerical issues, looking at different data smoothing methods, looking at adding marginal decreasing mortality, and fleshing out the mathematical analysis around R0 as outlined in our initial proposal.
\
